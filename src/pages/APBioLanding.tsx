...
const UNIT_DATA: { number: number; title: string; description: string }[] = [
  { number: 1, title: "Chemistry of Life",                description: "Water, macromolecules, and the molecules of life" },
  { number: 2, title: "Cells",                            description: "Cell structure, membranes, and transport" },
  { number: 3, title: "Cellular Energetics",              description: "Photosynthesis, cellular respiration, and fermentation" },
  { number: 4, title: "Cell Communication and Cell Cycle", description: "Signal transduction, mitosis, and regulation" },
  { number: 5, title: "Heredity",                         description: "Meiosis, Mendelian genetics, and non-Mendelian patterns" },
  { number: 6, title: "Gene Expression and Regulation",   description: "DNA replication, transcription, translation, and epigenetics" },
  { number: 7, title: "Natural Selection",                description: "Evolution, population genetics, and phylogenetics" },
  { number: 8, title: "Ecology",                          description: "Populations, communities, ecosystems, and conservation" },
];

const LEVELS = [
  { min: 0, label: "Organism",     marker: "🦠" },
  { min: 1, label: "Cell",         marker: "🧫" },
  { min: 2, label: "Tissue",       marker: "🧬" },
  { min: 3, label: "Organ",        marker: "🫀" },
  { min: 5, label: "System",       marker: "🌿" },
  { min: 7, label: "Ecosystem",    marker: "🌎" },
  { min: 8, label: "AP Scholar",   marker: "🏆" },
];
      ...
      <section className="mx-auto max-w-md space-y-3">
        ...
        {UNIT_DATA.map(({ number, title, description }) => {
          ...
          return (
            <button
              key={number}
              onClick={() => active && navigate(`/apbio/unit/${number}`)}
              disabled={!active}
              className={cn(
