...
const UNIT_DATA: { number: number; title: string; description: string }[] = [
  { number: 1, title: "Foundations of American Democracy",       description: "Constitutionalism, liberty, and civic participation" },
  { number: 2, title: "Interactions Among Branches",             description: "Checks, balances, and separation of powers" },
  { number: 3, title: "Civil Liberties and Civil Rights",        description: "Constitutional protections and landmark cases" },
  { number: 4, title: "Political Ideologies and Beliefs",        description: "Public opinion, socialization, and ideology" },
  { number: 5, title: "Political Participation",                 description: "Elections, voting, parties, and interest groups" },
];

const LEVELS = [
  { min: 0, label: "Citizen",      marker: "📜" },
  { min: 1, label: "Delegate",     marker: "🗳️" },
  { min: 2, label: "Legislator",   marker: "🏛️" },
  { min: 3, label: "Senator",      marker: "⚖️" },
  { min: 4, label: "Justice",      marker: "🔏" },
  { min: 5, label: "Chief Justice",marker: "🇺🇸" },
];
    ...
    <main className="min-h-screen bg-background px-4 py-12">
      ...
      <header className="mb-10 text-center animate-fade-in">
        ...
        <p className="mt-2 text-base text-muted-foreground">
          AP US Government & Politics · 5 Units
        </p>
      </header>
        ...
        >
          ...
          {completed.length < TOTAL_UNITS && (
            <p className="mt-2 text-[11px] text-muted-foreground">
              ...
              {LEVELS.find((l) => l.min > completed.length)?.label ?? "Chief Justice"}
            </p>
          )}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-md space-y-3">
        ...
        {UNIT_DATA.map(({ number, title, description }) => {
          ...
          return (
            <button
              key={number}
              onClick={() => active && navigate(`/apgov/unit/${number}`)}
              disabled={!active}
              className={cn(
